...
function findLetterFreqs(word) {
    ...
    for (; i < l; i++) {
        if (letterFreq[letters[i]]) letterFreq[letters[i]]++
        else letterFreq[letters[i]] = 1
    }
...
function decrementOrDelete(node, collection) {
    ...
    if (collection[node] > 1) collection[node]--
    else delete collection[node]
}
...
function findAnagrams(rack, userOptions, callback) {
    ...
    try {
        ...
        function searchTrie(freq, cur, trail) {
            for (var node in cur) {
                var isWildcard =
                    freq[node] === undefined && freq['?'] !== undefined

                if (
                    freq.total > 0 &&
                    (freq[node] !== undefined || (isWildcard && node !== '$'))
                ) {
                    ...
                    var localTrail = trail + node,
                        ...
                        localTrailLength = localTrail.replace(
                            /<\/*em>|\*/g,
                            ''
                        ).length
                    ...
                    // valid anagram?
                    if (val === 0 || val.$ === 0) {
                        if (groupedMatches[localTrailLength] === undefined)
                            groupedMatches[localTrailLength] = []
                        groupedMatches[localTrailLength].push(localTrail)
                        groupedMatches.count++
                    }

                    // reeecurse
                    if (localFreq.total > 0 && recurse)
                        searchTrie(localFreq, val, localTrail)
                }
            }
        }
...
function cliWildcardHelper(rack) {
    return rack.replace(/\^/g, '?')
}

function flexibleFindAnagrams(rack, argh, callback) {
    switch (typeof argh) {
        case 'function':
...
export { createDictionary, init, cliWildcardHelper, findAnagrams }
